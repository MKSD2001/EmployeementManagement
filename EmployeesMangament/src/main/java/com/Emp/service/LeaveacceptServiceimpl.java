...
public class LeaveacceptServiceimpl implements LeaveacceptService {
	
	private static final org.jboss.logging.Logger logger = org.jboss.logging.Logger
			.getLogger(LeaveacceptServiceimpl.class);
	
	// Create a new leave accept by a 
    @Override
	public Leave_acceptDTO createLeaveAccept(Leave_acceptDTO leaveAccept) {
    ...
    @Override
    public Leave_acceptDTO readLeaveAccept(int leaveRequestId) {
        try (Session session = Helper.getSession()) {
            CriteriaBuilder criteriaBuilder = session.getCriteriaBuilder();
            CriteriaQuery<Leave_acceptDTO> criteriaQuery = criteriaBuilder.createQuery(Leave_acceptDTO.class);
            Root<Leave_acceptDTO> root = criteriaQuery.from(Leave_acceptDTO.class);

            criteriaQuery.select(root)
                .where(criteriaBuilder.equal(root.get("leaveRequest").get("leaveRequestId"), leaveRequestId));

            Leave_acceptDTO leaveAccept = session.createQuery(criteriaQuery).uniqueResult();
     ...
     @Override
	public Leave_acceptDTO updateLeaveAccept(int Leave_acceptId, Leave_acceptDTO leaveAccept) {
		try (Session session = Helper.getSession()){
			
		Leave_acceptDTO upleaveacc = (Leave_acceptDTO) session.load(Leave_acceptDTO.class,Leave_acceptId  );
	...
	public void deleteLeaveAccept(int leaveAcceptId)  {
		try (Session session = Helper.getSession()){
			...
			Leave_acceptDTO delleaveacc = (Leave_acceptDTO) session.load(Leave_acceptDTO.class,leaveAcceptId);
